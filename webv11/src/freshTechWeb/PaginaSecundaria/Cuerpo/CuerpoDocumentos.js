...
class CuerpoDocumentos extends React.Component {
    ...
    .then(response =>{
      ...
      for (var i= 0; i < this.miListaC.listaC.length; i++){
        ...
        if(elemento != "sin categorias disponibles"){
          if(this.copiaLista.listaCopia[i] == this.fileEdit.categoria){
            console.log("EXISTE");
            existe = true;
            id = i;
          }
  ...
  actualizarFile=async(value1,value2)=>{
    ...
    console.log("NombreAnt: ", this.fileEdit.nombre);
    console.log("Nombre: ", this.state.nombreDoc);
    console.log("Categoria: ", value1);
    console.log("Caducidad: ", this.state.expiracionDoc);
    console.log("Creacion: ", this.state.creacionDoc);
    
    const datos = {nuevoNombre:this.state.nombreDoc, categoria:value1,fechacreacion:this.state.creacionDoc,fechacaducidad:this.state.expiracionDoc,nombreAntiguo:this.fileEdit.nombre,actualizaImagen:'no'};
    ...
    .then(response =>{
      ...
      this.setState({cargaContenido:true},() => this.togglePopup1());
    })
    .catch(error=>{
  ...
  handleSubmitEdit =e => {
    ...
    this.actualizarFile(cat,this.state.token);
        ...
        <Popup
          ...
          cuerpo={
            ...
            <form className="pup" onSubmit={this.handleSubmitEdit}>
              ...
              <br></br>
              <input type='submit' className="btn btn-primary mr-2" value='Actualizar'/>
