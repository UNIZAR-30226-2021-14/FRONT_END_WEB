...
import ArrayListIMG from '../../Desplegable/ArrayListIMG.js'
...
class CuerpoImagenes extends React.Component {
  ...
  constructor(props){
    ...
    this.state = {
      ...
      categoria:'cat',
      vacio:true,
      ordenarPor:localStorage.getItem('ordenarPorI'),
      ordenarDe:localStorage.getItem('ordenarDeI'),
  ...
  selectImagenes=async(value)=>{
    ...
    console.log("Ordenar de img: ",this.state.ordenarDe);
    //const query = {ordenarPor:this.state.ordenarPor,ordenarDe:this.state.ordenarDe};
    //const headers = {'Authorization':`Bearer ${value}`};
    const config = {
      headers: {'Authorization':`Bearer ${value}`},
    ...
    .then(response =>{
      ...
      if(lista.length == 0){
        ...
        this.setState({listadoImagenes:lista,cargaContenido:false},() =>this.cargarImagenes());
    })
  }
  cargarImagenes(){
  ...
  selectImgDetails=async(value1,value2)=>{
    ...
    .then(response =>{
      ...
      this.imgEdit.activacion=response.data.fechacreacion;
      this.imgEdit.caducidad=response.data.fechacaducidad;
      this.imgEdit.url=response.data.dominio;
      this.imgEdit.nombre=value1;
  ...
  render () {
    ...
    return (
        ...
        <div className="bloqueArray">
          ...
          <div className="filtro">
            <ArrayListIMG tipo={false} valores={this.miListaV.listaV} contenido={"imagenes"}/>
          </div>
        </div>
      </div>
      <br></br><br></br>
              ...
              <>
               <form className="pup" onSubmit={this.handleSubmit}>
                <span className="par">
                  ...
                  <pre>      </pre>
                  <ArrayListIMG tipo={true} valores={this.miListaC.listaC}/>
