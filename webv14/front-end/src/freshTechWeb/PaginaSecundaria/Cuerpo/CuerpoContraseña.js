...
class CuerpoContraseña extends React.Component {
  ...
  crearContrasenya=async(value)=>{
    ...
    if(localStorage.getItem('categoria') == null){
      if(this.miListaC.listaC[0] == "sin categorias disponibles"){
        cat = null
      }else{
        cat = this.miListaC.listaC[0];
      }
      
    }else{
      cat = localStorage.getItem('categoria');
    }
    const datos = {concreteuser:this.state.usuario,concretepasswd:this.state.contrasenya, dominio:this.state.url,fechacreacion:this.state.fecha_actual,fechacaducidad:this.state.fecha_caducidad,nombre:this.state.nombre,categoria:cat};
